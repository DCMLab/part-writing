...
maxEvent :: Voice v =>
            Model v -> EEvent v -> [EEvent v] -> State v -> Context v -> V.Vector (Event v)
         -> IO (EEvent v, State v)
...
mapEstimateNotewise :: Voice v => AutoOpts v -> Maybe (Piece v) -> Model v -> Int -> [v] -> IO (Piece v)
...
maxNote :: Voice v =>
  State v -> [EEvent v] -> Context v -> M.Map v (V.Vector (Feature v), ModelParams) ->
  GenIO -> V.Vector Pitch -> V.Vector v -> IO (EEvent v, State v)
...
bestEstimate :: Voice v => AutoOpts v -> IO [(Piece v)] -> Model v ->  IO (Piece v, Double)
bestEstimate opts estimates model = do
  (p1:ps) <- estimates
  pure $ foldl comp (p1, evalPieceUnnormLog opts p1 model) ps
  where comp (old, oldScore) new =
          let newScore = evalPieceUnnormLog opts new model in
  ...
  let evs     = extendPiece piece
      ...
      goEstimate n evs
        ...
        | otherwise       = do
            ...
            putStrLn $ printf ", meanLogPot = % 7.5f" (meanLogPotential opts p model)
...
estimateGibbsNotes :: (Voice v) =>
                      AutoOpts v -> [v] -> Piece v -> Model v -> Int -> (Double -> Double)
                   -> IO (Piece v)
estimateGibbsNotes opts fixedVoices piece =
  estimateGibbsAnnealing opts step piece
...
estimateGibbsNotes' :: (Voice v) =>
                       AutoOpts v -> [v] -> [V.Vector Pitch] -> Piece v -> Model v -> Int -> (Double -> Double)
                    -> IO (Piece v)
estimateGibbsNotes' opts fixedVoices groups piece =
  estimateGibbsAnnealing opts step piece
  where ctx = mkDefaultCtx opts (keySignature $ pieceMeta piece)
        vVec = V.fromList (voiceList \\ fixedVoices)
