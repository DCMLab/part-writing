...
module VoiceLeading.Distribution (
  ...
  -- * Evaluation
  sumFeaturesP, countFeaturesP,
  sumFeaturesM, countFeaturesM,
...
sumFeaturesP :: (Foldable t, Num a, VU.Unbox a) =>
               V.Vector b -> t (VU.Vector a) -> VU.Vector a
sumFeaturesP !feats !counts = foldl' (VU.zipWith (+)) (VU.convert $ 0 <$ feats) counts

sumFeaturesM :: (Foldable t, Num a, VU.Unbox a, PrimMonad m) =>
                V.Vector b -> t (VU.Vector a) -> m (VU.Vector a)
sumFeaturesM !feats !counts = do
  accum <- VUM.replicate n 0
  forM_ counts (addVec accum)
  VU.unsafeFreeze accum
  where n = V.length feats
...
countFeaturesP :: Voice v => AutoOpts v -> Piece v -> V.Vector (Feature v) -> FeatureCounts
countFeaturesP opts piece feats = sumFeaturesP feats $ runFeaturesOn opts piece feats

countFeaturesM :: (Voice v, PrimMonad m) => AutoOpts v -> Piece v -> V.Vector (Feature v) -> m FeatureCounts
countFeaturesM opts piece feats = sumFeaturesM feats $ runFeaturesOn opts piece feats
...
evalModelUnnormLog fs thetas
  | VU.length fs == VU.length thetas = VU.sum (VU.zipWith (*) fs thetas)
  | otherwise =
    error $ "feature counts and params vectors must have the same length: |fs| = "
    ++ show (VU.length fs) ++ ", |thetas| = " ++ show (VU.length thetas)
...
meanLogPotentialN opts ps model =
  let nevs   = sum $ map pieceLen ps
      logpot = sum $ map (flip (evalPieceUnnormLog opts) model) ps in
...
meanFeatCounts opts piece (Model nfeats _) =
  VU.map (/ fromIntegral (pieceLen piece)) (countFeaturesP opts piece (unname nfeats))

-- | Returns for each feature in the given 'Model'
--   the average value of that feature per event in the given 'Piece's.
--   Like 'meanFeatCounts' but for several pieces.
meanFeatCountsN :: Voice v => AutoOpts v -> [Piece v] -> Model v -> FeatureCounts
meanFeatCountsN opts ps (Model nfeats _) = expectedFeats opts ps (unname nfeats)
...
expectedFeats :: Voice v => AutoOpts v -> [Piece v] -> V.Vector (Feature v) -> FeatureCounts
...
expectedFeatsP :: (Voice v, S.MonadAsync m) =>
                  AutoOpts v -> [Piece v] -> V.Vector (Feature v) -> m FeatureCounts
expectedFeatsP !opts !pieces !features = do
  let pieceCounts = S.asyncly $ S.mapM (pure . flip (countFeaturesP opts) features) $
                    S.fromList $ force pieces
